...
var a = "document.getElementById('ourSearchKey1').innerText= JSON.stringify(window.sessionStorage)";
...
function initiatAutoMMT(){
  // console.log("called initiatAutoMMT ");
  var cur_url = window.location.href;
  ...
  if($("#couponClick").length == 0 && cur_url.split("/international/raw/index.html").length > 1){
    setTimeout(initiatAutoMMT, 500);
  }
}
initiatAutoMMT();

function autoCouponInititate(){
  if(SessionKey != "" && $("#couponClick").length == 0){
