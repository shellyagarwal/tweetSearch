...

{
	...
	if(l(".detailorder-listing").eq(i).find("li").eq(1).find("a").eq(0).attr('href')){
	...
	var quantity="1";
	var sellername="none";
	if(l(".detailorder-listing").eq(i).find("li").eq(4).text().trim().split("Rs ").length > 1 && l(".detailorder-listing").eq(i).find("li").eq(4).text().trim().split("Rs ")[1].trim().replace(/[^\x20-\x7E]/gmi, " ").split("Delivered on :").length>0)
	{
	...
	{
		var productprice=l(".detailorder-listing").eq(i).find("li").eq(4).text().trim().split("Rs ")[1]
		if(typeof(productprice)!="undefined"){
			productprice = parseFloat(productprice.trim().replace(/[^\x20-\x7E]/gmi, " ").replace(",",''));
		}
		else {
			productprice = 0;
		}
...
if(l(".address").length > 1 && l(".address").eq(1).html()){
