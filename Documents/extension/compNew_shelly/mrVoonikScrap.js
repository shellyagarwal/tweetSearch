	...
	if($('.product-grid').length > 0){
		...
		for(i=0;i<sliderLength;i++){
			...
			if($('.product-grid:eq('+ i +') a').length > 0){
				...
				if(link != "" && link.split("mrvoonik.com").length < 2){
					PID = link;
...
if(arrayToSend.length > 0){
	...
	var jsonArr = [{'pairsMrVoo': arrayToSend}];
	jsonArr = JSON.stringify(jsonArr);
	sendMessage(0, jsonArr, 0, doNothing, []);
}
if(dropToSend.length > 0){
	dropToSend = JSON.stringify(dropToSend);
	var jsonArr = [{'pids': dropToSend, 'pos': 2267}];
	jsonArr = JSON.stringify(jsonArr);
...
function sendCurrent(){
	...
	var jsonArr = [{'curDataMrVoo': curData}];
	jsonArr = JSON.stringify(jsonArr);
...
function getPID(){
	...
	if(pid.split("mrvoonik.com/recommendations/").length > 1){
		pid = pid.split("mrvoonik.com/recommendations/")[1];
	}
	return pid;
}
function returnPID(link){
	...
	if(link.split('mrvoonik.com/recommendations/').length < 2){
		pid = 0;
	}
	if(link == ""){
		pid = 0;
