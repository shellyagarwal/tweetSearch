...
else if(siteName.split("MRVOONIK.").length>1){
   pos = 2267;
   cur_site = "MRVOONIK";
}
else if(siteName.split(".VOONIK.").length>1){
   pos = 2266;
   cur_site = ".VOONIK";
}
return pos;
...
function addCoupons(data){
   ...
   if (data == 0){
      document.getElementById('currentStatus').innerText = 'NO COUPONS FOUND';
      ...
      $('#currentStatus').html('<span style="width: 100px">ALL ('+sidebar.total+')</span>'
   ...
   $('#toAddCouponsHere').html(coupons_html);
 ...
 if(document.getElementById("hc-toggle") != null){
  ...
  var cur_pos = getCurrentPosition(window.location.href);
    ...
    if(cur_pos!=0){
     ...
     document.getElementById('titleChange').innerText = (cur_site + " Coupons");
     document.getElementsByClassName('hc-code-sub')[0].innerText = ("ADD " + cur_site.toUpperCase() + " COUPONS");
